...
<div class="sub-top-row"><div class="sub-field-grp"><label class="sub-lbl" for="air-rank-inp">等級</label><input type="number" id="air-rank-inp" class="sub-inp" value="1" min="1" max="50"><span class="sub-exp-next" id="air-exp-next"></span><span class="sub-exp-badge" id="air-exp-badge" data-tip-wide="1" data-tip="" title="查看全部等級經驗需求">i</span></div><button id="air-search-btn" class="sub-search-btn" style="margin-left:auto">搜　尋</button><button id="air-clear-btn" class="sub-clear-btn">清除篩選</button><button id="air-view-toggle" class="sub-view-btn" type="button">切換為精簡模式</button></div>
...
<div id="air-reverse-panel" style="display:none"></div>
  ...
  const mkLootFilter = (wid, arr) => {
    ...
    const allOpts = allIds.map(id => ({ id, name: getName(id), isKey: AIR_KEY_LOOT.has(id) })).sort((a, b) => { if (a.isKey !== b.isKey) return a.isKey ? -1 : 1; return a.name.localeCompare(b.name, 'zh-TW'); });
    const renderDD = q => {
  ...
  const ensureAirWorker = () => {
    ...
    try {
      const w = new Worker('air-worker.js');
  ...
  <div id="air-rev-results-stat" style="display:none"><div class="sub-empty">輸入需求值與擁有的部件後按搜尋</div></div>`;
    ...
    const allItemOpts = Object.keys(AIR_ITEM_SECTORS).map(id => ({ id: +id, name: getName(+id), isKey: AIR_KEY_LOOT.has(+id) })).sort((a, b) => { if (a.isKey !== b.isKey) return a.isKey ? -1 : 1; return a.name.localeCompare(b.name, 'zh-TW'); });
