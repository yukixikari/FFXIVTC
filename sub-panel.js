...
<div class="sub-top-row"><div class="sub-field-grp"><label class="sub-lbl" for="sub-rank-inp">等級</label><input type="number" id="sub-rank-inp" class="sub-inp" value="1" min="1" max="145"></div><button id="sub-search-btn" class="sub-search-btn" style="margin-left:auto">搜　尋</button><button id="sub-clear-btn" class="sub-clear-btn">清除篩選</button><button id="sub-view-toggle" class="sub-view-btn" type="button">切換為精簡模式</button></div>
...
<div id="sub-reverse-panel" style="display:none"></div>
    ...
    const mkLootFilter=(wid,arr)=>{
      ...
      const allOpts=allIds.map(id=>({id,name:getName(id),isKey:KEY_LOOT.has(id)})).filter(o=>!o.name.startsWith('ID:')).sort((a,b)=>{if(a.isKey!==b.isKey)return a.isKey?-1:1;return a.name.localeCompare(b.name,'zh-TW');});
      const renderDD=q=>{
        dd.innerHTML='';q=(q||'').trim().toLowerCase();let inK=false,inO=false;
        allOpts.forEach(o=>{
          if(q&&!o.name.toLowerCase().includes(q))return;
          if(o.isKey&&!inK){inK=true;const g=document.createElement('div');g.className='sub-combo-grp';g.textContent='重點物品';dd.appendChild(g);}
          if(!o.isKey&&!inO){inO=true;const g=document.createElement('div');g.className='sub-combo-grp';g.textContent='一般物品';dd.appendChild(g);}
    ...
    const ensureSubWorker=()=>{
      ...
      try{
        const w=new Worker('sub-worker.js');
  ...
  <div id="sub-rev-results-stat" style="display:none"><div class="sub-empty">輸入需求值與擁有的部件後按搜尋</div></div>`;
      ...
      const allItemOpts=Object.keys(SUB_ITEM_SECTORS).filter(id=>typeof ITEM_DB!=='undefined'&&ITEM_DB[id]).map(id=>({id:+id,name:getName(+id),isKey:KEY_LOOT.has(+id)})).sort((a,b)=>{if(a.isKey!==b.isKey)return a.isKey?-1:1;return a.name.localeCompare(b.name,'zh-TW');});
      ...
      const renderItemPicks=()=>{
        ...
        selItems.forEach(itemId=>{
          ...
          cands.forEach(c=>{
            ...
            const o=document.createElement('option');o.value=c.sec;o.textContent=`(${MAP_NAMES[s[1]]}) ${s[2]} ${s[4]}（探索需${TIER_LABEL2[c.tier]}${surThr!=null?' '+surThr:''}）`;
      ...
      const renderItemDD=q=>{
        iDd.innerHTML='';q=(q||'').trim().toLowerCase();let inK=false,inO=false;
        allItemOpts.forEach(o=>{
          if(q&&!o.name.toLowerCase().includes(q))return;
          if(o.isKey&&!inK){inK=true;const g=document.createElement('div');g.className='sub-combo-grp';g.textContent='重點物品';iDd.appendChild(g);}
          if(!o.isKey&&!inO){inO=true;const g=document.createElement('div');g.className='sub-combo-grp';g.textContent='一般物品';iDd.appendChild(g);}
