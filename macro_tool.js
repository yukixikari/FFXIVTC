...
var TEMPLATES=[
  ...
  {id:'marking',icon:'🏷',label:'目標標記',desc:'設定攻擊順序標記',
   lines:[
  ...
  {id:'kaomoji',icon:'🐱',label:'自訂顏文字',desc:'多行文字圖案，可自由修改',
   lines:[
  ...
  {id:'em',icon:'🎭',label:'感情表現',desc:'第三人稱自訂文字，可搭配情感動作',
   lines:[
...
/* ════════════════════════════════════════════════
   UTILITIES
════════════════════════════════════════════════ */
function cc(s){var n=0;for(var i=0;i<s.length;i++)n+=s.codePointAt(i)>0x7F?3:1;return n;}
...
function buildLinesInner(line){
  ...
  switch(line.type){
    ...
    case 'tell':{
      return['/t '+(p.target||'玩家名稱@伺服器名')+' '+(p.message||'')];
    }
    case 'reply':{
    ...
    case 'em':{
      ...
      if(p.addMotion&&p.motionEmote){
        var em_name=p.motionEmote==='custom'?(p.motionCustom||'行禮指令名'):p.motionEmote;
    ...
    case 'companionaction':{
      ...
      // Recast notify
      if(p.recastNotify&&sname&&sname!=='技能名稱'){
    ...
    case 'emote':{
      var emote_out=[];
      var ename=p.emoteName==='custom'?(p.emoteCustom||'情感動作指令名'):p.emoteName||'bow';
    ...
    case 'mount':{
      var mn2=p.mountName||'坐騎名稱';
      return['/mount "'+mn2+'"'];
    }
    case 'minion':{
      var mi2=p.minionName||'寵物名稱';
      return['/minion "'+mi2+'"'];
    }
    case 'battlemode':{
    ...
    case 'micon':{
      ...
      return[mit==='gearset'?'/micon '+(p.miconName||'1')+' gearset':'/micon "'+(p.miconName||'技能名稱')+'" '+mit];
    }
...
function toolHTML(){
  return '<h2 class="unified-gold-header small">⚙ 巨集建立工具</h2>'
    ...
    +'<p class="mt-intro">FF14 巨集上限 <b>15 行</b>，每行上限 <b>180 字元</b>（中文字每個佔3字元，英文/符號/空格各佔1字元）。工具即時計算並提示問題。每一行均可自由修改、增刪及調換順序。</p>'
    ...
    if(out.length>MAX){
      ...
      sa.innerHTML='<div class="mt-split-note">⚠ 此巨集超過 '+MAX+' 行，已自動分割成兩個巨集。兩個都需分別複製使用，請依序執行巨集1 → 巨集2。</div>'
        +'<div class="mt-dual">'
          ...
          +'<div class="mt-dual-col"><div class="mt-dual-hdr">巨集 2（'+splitData.m2.length+'/'+MAX+' 行）</div><pre class="mt-split-pre">'+esc(splitData.m2.join('\n'))+'</pre><button class="mt-split-copy" data-which="2">複製巨集 2</button></div>'
        +'</div>';
...
function updateRow(id){
  var line=lines.filter(function(l){return l.id===id;})[0];if(!line)return;
...
function doSplit(allLines){
  ...
  var m2=m2header.concat(rawM2).slice(0,MAX);
  return{m1:m1,m2:m2};
}
...
function bindRoot(){
  ...
  if(etBtn)etBtn.addEventListener('click',function(){
    ...
    if(previewEditable){
      ...
      var content2=ta2?ta2.value:'';
      wrap.innerHTML='<pre class="mt-preview" id="mt-preview">'+esc(content2)+'</pre>';
...
function bindLineEvents(){
  ...
  el.querySelectorAll('.ml-type-sel').forEach(function(sel){
    sel.addEventListener('change',function(e){
      var id=+sel.dataset.id;var line=lines.filter(function(l){return l.id===id;})[0];
  ...
  el.querySelectorAll('.mf-inp').forEach(function(inp){
    inp.addEventListener('input',function(){
      ...
      var line=lines.filter(function(l){return l.id===id;})[0];if(!line||!key)return;
  ...
  el.querySelectorAll('.ml-note').forEach(function(inp){
    inp.addEventListener('input',function(){
      var id=+inp.dataset.id;var line=lines.filter(function(l){return l.id===id;})[0];
  ...
  el.querySelectorAll('.ml-expand-btn').forEach(function(btn){
    btn.addEventListener('click',function(e){e.stopPropagation();
      var id=+btn.dataset.id;var line=lines.filter(function(l){return l.id===id;})[0];
      if(!line)return;
  ...
  el.querySelectorAll('.is-sel-common').forEach(function(btn){
    btn.addEventListener('click',function(e){e.stopPropagation();
      var id=+btn.dataset.id;var line=lines.filter(function(l){return l.id===id;})[0];if(!line)return;
