...
function subEnumerateCandidates(rank, mustInc, mustExcl, mustIncMaps, mustExclMaps, onlyMapIds) {
  const candidates = [];
  [...new Set(SUB_SECTORS.map(s => s[1]))].forEach(mapId => {
    ...
    for (let n = 0; n <= Math.min(maxExtra, topOpt.length); n++) { subCombos(topOpt, n).forEach(combo => {
      const secs = [...required, ...combo]; if (!secs.length) return;
      const best = subBestOrder(homeId, secs.map(s => s[0]));
...
function subEvaluateOwnedCandidates(rank, candidates, ownedRanks, mustLootIds, desiredMin, useTime, lootStrict, lootTier, lootMustMeet) {
  ...
  if (!needPerComboCheck) {
    ...
    return { routes: results.slice(0, 200), noParts: false };
  }
  ...
  results.sort((a, b) => b.epm - a.epm);
  return { routes: results.slice(0, 200), noParts: false, capped, totalCount: combos.length, searchedCount: combosForFiltered.length };
}
