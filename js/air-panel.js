...
app.buildAir = function () {
  ...
  /* 擁有的部件：路線搜尋（擁有部件模式）與配裝反查共用同一份記錄，架構比照 sub-panel.js。 */
  ...
  const PARTS_MODE_KEY = 'xiv-air-parts-mode';
  ...
  /* 表格式／卡片式檢視模式，架構比照 sub-panel.js。 */
  const VIEW_MODE_KEY = 'xiv-air-view-mode';
  let viewMode = 'table';
  try { if (localStorage.getItem(VIEW_MODE_KEY) === 'card') viewMode = 'card'; } catch (e) {}
  const SORT_PROP = { exp: 'exp', time: 'timeMins', epm: 'epm', itemEff: 'itemEff' };
  const SORT_LABEL = { exp: '經驗值', time: '時間', epm: 'EXP/分', itemEff: '物品效率' };
...
<div class="sub-top-row"><div class="sub-field-grp"><label class="sub-lbl" for="air-rank-inp">等級</label><input type="number" id="air-rank-inp" class="sub-inp" value="1" min="1" max="50"><span class="sub-exp-next" id="air-exp-next"></span><span class="sub-exp-badge" id="air-exp-badge" data-tip-wide="1" data-tip="" title="查看全部等級經驗需求">i</span></div><button id="air-search-btn" class="sub-search-btn" style="margin-left:auto">搜　尋</button><button id="air-parts-mode-toggle" class="sub-view-btn" type="button">切換「擁有部件」選項</button><button id="air-view-toggle" class="sub-view-btn" type="button">切換為精簡模式</button><button id="air-map-btn" class="sub-view-btn" type="button">探索地圖</button><button id="air-clear-btn" class="sub-clear-btn">清除篩選</button></div>
<div class="sub-parts-row" id="air-parts-select-row">
...
<div class="sub-stats-bar" id="air-stats-bar">
...
<div class="sub-owned-route-block" id="air-owned-route-block" style="display:none">
  <div class="sub-owned-hdr-row"><label class="sub-lbl">擁有的部件（與「配裝反查」共用同一份記錄，勾選後搜尋會忽略維修成本，自動找出效率最高的路線＋建議配裝，計算耗時較久）</label><div class="sub-owned-actions"><button type="button" id="air-owned-route-all" class="sub-owned-mini-btn">全選</button><button type="button" id="air-owned-route-none" class="sub-owned-mini-btn">全部清除</button></div></div>
  <div id="air-owned-route-grid"></div>
  <div class="sub-owned-tier-row"><label class="sub-lbl" for="air-owned-tier">獲得物品門檻</label><select id="air-owned-tier" class="sub-sel"><option value="none">不限（純比效率，較快）</option><option value="normal">全程每一站至少「達標」</option><option value="best">全程每一站都「最佳」</option></select><span class="sub-owned-tier-note">開啟門檻篩選時，搜尋範圍變大，會比純比效率慢一些</span></div>
</div>
...
<div class="sub-filter-col"><label class="sub-lbl">必須獲得物品</label><div class="sub-multisel-wrap" id="air-loot-wrap"></div><label class="sub-chk-lbl sub-loot-strict-lbl"><input type="checkbox" id="air-loot-strict"> 每個目的地都至少有其中一項物品</label><label class="sub-chk-lbl sub-loot-meet-lbl" title="只計入探索性能已經達到取得門檻的物品；還沒達標、理論上有機率但目前配裝拿不到的不算數"><input type="checkbox" id="air-loot-meet"> 忽略不達標的結果</label><div class="sub-loot-sort-row" id="air-loot-sort-row" style="display:none"><label class="sub-lbl" for="air-loot-sort">針對獲得物品的排序依據</label><select id="air-loot-sort" class="sub-sel"><option value="exp">EXP 效率（預設）</option><option value="coverage">物品覆蓋效率（短時間內湊齊優先）</option><option value="count">物品數量效率（拿越多越好）</option></select></div></div>
<div id="air-range-disp" class="sub-range-disp">目前航行距離：—</div>
  </div>
</div>
<div class="sub-viewmode-row"><span class="sub-lbl">顯示方式</span><div class="sub-seg" id="air-viewmode-seg"><button type="button" class="sub-seg-btn" data-mode="table">表格式</button><button type="button" class="sub-seg-btn" data-mode="card">卡片式</button></div></div>
...
<div id="air-reverse-panel" style="display:none"></div>
  ...
  if (savedFilter) {
    ...
    const meetChk = root.querySelector('#air-loot-meet'); if (meetChk) meetChk.checked = !!savedFilter.lootMustMeet;
    const letterChk = root.querySelector('#air-letter'); if (letterChk) letterChk.checked = !!savedFilter.letterOnly;
  }

  /* ── 路線搜尋的「擁有部件」模式（架構比照 sub-panel.js）── */
  const renderOwnedRouteGrid = () => {
    const gridEl = root.querySelector('#air-owned-route-grid'); if (!gridEl) return;
    ...
    ['hull', 'rigging', 'forecastle', 'aftcastle'].forEach(slot => {
      ...
      AIR_PARTS[slot].forEach(p => {
        ...
        html += `<label class="sub-owned-item"><input type="checkbox" data-slot="${slot}" data-rank="${p[1]}" ${checked}> ${AIR_RANK_CODE[p[1]] || p[1]} — ${p[2]}</label>`;
      });
      html += '</div>';
    });
    html += '</div>';
  ...
  root.querySelector('#air-owned-route-all')?.addEventListener('click', () => {
    ['hull', 'rigging', 'forecastle', 'aftcastle'].forEach(slot => { ownedParts[slot] = AIR_PARTS[slot].map(p => p[1]); });
    saveOwnedParts(); renderOwnedRouteGrid();
  });
  root.querySelector('#air-owned-route-none')?.addEventListener('click', () => {
    ['hull', 'rigging', 'forecastle', 'aftcastle'].forEach(slot => { ownedParts[slot] = []; });
    saveOwnedParts(); renderOwnedRouteGrid();
  });
  const applyPartsMode = () => {
    const selRow = root.querySelector('#air-parts-select-row');
    const ownedBlock = root.querySelector('#air-owned-route-block');
    const statsBar = root.querySelector('#air-stats-bar');
    const toggleBtn = root.querySelector('#air-parts-mode-toggle');
    ...
    if (isOwned) renderOwnedRouteGrid();
  };
  root.querySelector('#air-parts-mode-toggle')?.addEventListener('click', () => {
  ...
  mkSecFilter('air-inc-wrap', mustInc); mkSecFilter('air-excl-wrap', mustExcl);
  /* 排序依據選單：只有勾了「必須獲得物品」才顯示，架構比照 sub-panel.js。 */
  const updateLootSortVisibility = () => {
    const row = root.querySelector('#air-loot-sort-row');
    if (row) row.style.display = mustLootIds.length ? '' : 'none';
    if (!mustLootIds.length) { const sel = root.querySelector('#air-loot-sort'); if (sel) sel.value = 'exp'; }
  };
  mkLootFilter('air-loot-wrap', mustLootIds, updateLootSortVisibility);
  root.querySelector('#air-loot-sort')?.addEventListener('change', () => {
    const sel = root.querySelector('#air-loot-sort');
    sortKey = sel.value === 'exp' ? 'epm' : 'itemEff'; sortDir = -1;
    root.querySelector('#air-search-btn')?.click();
  });
  ...
  const doSearch = () => {
    ...
    let st = null;
    if (!owned) {
      st = getStats(); if (!st) { root.querySelector('#air-results').innerHTML = '<div class="sub-empty">請先選擇所有部件</div>'; return; }
    } else {
      const hasAll = ['hull', 'rigging', 'forecastle', 'aftcastle'].every(slot => (ownedParts[slot] || []).length > 0);
      if (!hasAll) { root.querySelector('#air-results').innerHTML = '<div class="sub-empty">請至少為每個部位（船體／舾裝／船首／船尾）勾選一種擁有的部件</div>'; return; }
    }
    const lootStrict = root.querySelector('#air-loot-strict')?.checked || false;
    const lootMustMeet = root.querySelector('#air-loot-meet')?.checked || false;
    ...
    const buildLootCell = (secIds, curStats, maxShow) => {
      ...
      const MAX_SHOW = maxShow || 4; let html = '';
      [1, 2, 3].forEach(t => {
    ...
    renderRoutes = (routes) => {
      ...
      const lootSortSel = root.querySelector('#air-loot-sort');
      ...
      if (showItemEff) {
        routes.forEach(r => {
          ...
          r.secIds.forEach(id => {
            const avail = airLootAvailableAt(id, rowStats, lootMustMeet);
      ...
      /* ===== 卡片式（票根式）檢視，架構比照 sub-panel.js ===== */
      if (viewMode === 'card') {
        const heroOf = r => {
          if (sortKey === 'exp') return { val: r.exp.toLocaleString(), label: '經驗值' };
          if (sortKey === 'time') return { val: r.timeStr, label: '耗時' };
          if (sortKey === 'itemEff' && showItemEff) return { val: r.itemEff.toFixed(2) + '／時', label: lootSortMode === 'coverage' ? '物品覆蓋效率' : '物品數量效率' };
          return { val: r.epm.toLocaleString(), label: 'EXP/分' };
        };
        const mkCard = (r, idx) => {
          const rank = idx + 1;
          const rowStats = r.build ? { sur: r.build.st.sur, ret: r.build.st.ret, fav: r.build.st.fav } : curStats;
          const hero = heroOf(r);
          const secondary = [`Lv${r.minRank}`];
          if (hero.label !== '經驗值') secondary.push(`經驗值 ${r.exp.toLocaleString()}`);
          if (hero.label !== '耗時') secondary.push(r.timeStr);
          if (hero.label !== 'EXP/分') secondary.push(`EXP/分 ${r.epm.toLocaleString()}`);
          secondary.push(`距離${r.range}／燃料${r.tank}`);
          const buildTag = r.build ? (() => { const b = r.build; const tip = `船體：${b.hull[2]}\n舾裝：${b.rigging[2]}\n船首：${b.forecastle[2]}\n船尾：${b.aftcastle[2]}\n承載力：${b.cost}/${b.cap ?? '—'}`; const code = p => AIR_RANK_CODE[p[1]] || p[1]; return `<div class="sub-card-tag" data-tip="${tip}"><span>⚓</span> ${code(b.hull)}／${code(b.rigging)}／${code(b.forecastle)}／${code(b.aftcastle)}</div>`; })() : '';
          const thrHtml = buildThresholdStrip(r.secIds, rowStats);
          const lootHtml = buildLootCell(r.secIds, rowStats, 10);
          return `<div class="sub-route-card${rank === 1 ? ' sub-card-top' : ''}">` +
            (rank === 1 ? '<div class="sub-card-ribbon"><span>#1</span></div>' : '') +
            `<div class="sub-card-left">` +
              `<div class="sub-card-left-top">` +
                (rank !== 1 ? `<div class="sub-card-rank">第 ${rank} 名</div>` : '') +
                `<div class="sub-card-hero-val">${hero.val}</div>` +
                `<div class="sub-card-hero-underline"></div>` +
                `<div class="sub-card-hero-label">${hero.label}</div>` +
                `<div class="sub-card-secondary">${secondary.join('<br>')}</div>` +
                buildTag +
              `</div>` +
              `<div class="sub-card-tear"><div class="sub-card-tear-line"></div></div>` +
              `<div class="sub-card-dest">` +
                `<div class="sub-card-dest-label">目的地 ${r.secCount} 站</div>` +
                `<div class="sub-card-dest-letters">${r.secLetters || ''}</div>` +
                `<div class="sub-card-dest-path">${r.secStr}</div>` +
                `<div class="sub-thr-strip">${thrHtml}</div>` +
              `</div>` +
            `</div>` +
            `<div class="sub-card-divider"></div>` +
            `<div class="sub-card-right">` +
              `<div class="sub-card-items"><div class="sub-card-items-label">獲得物品</div>${lootHtml}</div>` +
            `</div>` +
          `</div>`;
        };
        const cardSortKeys = [{ k: 'epm', l: 'EXP/分' }, { k: 'exp', l: '經驗值' }, { k: 'time', l: '耗時' }];
        if (showItemEff) cardSortKeys.push({ k: 'itemEff', l: lootSortMode === 'coverage' ? '物品覆蓋效率' : '物品數量效率' });
        const sortPillsHtml = cardSortKeys.map(o => {
          const active = sortKey === o.k;
          const arrow = active ? (sortDir === -1 ? ' ↓' : ' ↑') : '';
          return `<button type="button" class="sub-card-sort-pill${active ? ' active' : ''}" data-sortkey="${o.k}">${o.l}${arrow}</button>`;
        }).join('');
        const CARD_BATCH = 30;
        const firstCards = sorted.slice(0, CARD_BATCH);
        let cardShown = firstCards.length;
        res.innerHTML = legendHtml + `<div class="sub-card-sort-row">${sortPillsHtml}</div><div class="sub-card-grid">` +
          firstCards.map((r, i) => mkCard(r, i)).join('') + `</div>`;
        /* 「智能」決定右欄物品顯示範圍，架構比照 sub-panel.js：量出左欄實際
           渲染高度，右欄照這個高度限制，超出的部分用淡出遮罩收起來。
           用 ResizeObserver 而不是量一次就算了，避免字型/圖片載入或路線
           文字換行造成左欄高度事後才變化時，右欄卻沒跟著更新。 */
        const MIN_ITEMS_H = 220, MAX_ITEMS_H = 520, FALLBACK_ITEMS_H = 320;
        const fitCardHeights = scope => {
          scope.querySelectorAll('.sub-route-card').forEach(card => {
            const left = card.querySelector('.sub-card-left');
            const items = card.querySelector('.sub-card-items');
            if (!items) return;
            const updateFade = () => {
              const overflowing = items.scrollHeight > items.clientHeight + 2;
              const atBottom = items.scrollTop + items.clientHeight >= items.scrollHeight - 2;
              items.classList.toggle('has-fade', overflowing && !atBottom);
            };
            const syncHeight = () => {
              const leftH = left ? left.offsetHeight : 0;
              const h = Math.max(MIN_ITEMS_H, Math.min(MAX_ITEMS_H, leftH || FALLBACK_ITEMS_H));
              card.style.setProperty('--items-max-h', h + 'px');
              updateFade();
            };
            if (!items.dataset.scrollBound) {
              items.addEventListener('scroll', updateFade);
              items.dataset.scrollBound = '1';
            }
            if (left && !card.dataset.roBound) {
              new ResizeObserver(syncHeight).observe(left);
              card.dataset.roBound = '1';
            }
            syncHeight();
          });
        };
        fitCardHeights(res);
        /* 展開「⋯等N種」之後物品清單變長，遮罩／捲動狀態要重新算一次。 */
        res.addEventListener('click', e => {
          if (e.target.closest('.loot-toggle')) setTimeout(() => fitCardHeights(res), 0);
        });
        res.querySelectorAll('.sub-card-sort-pill').forEach(btn => {
        ...
        if (sorted.length > cardShown) {
          const moreBtn = document.createElement('button');
          moreBtn.className = 'sub-more-btn';
          moreBtn.textContent = `顯示更多（已顯示 ${cardShown} ／ 共 ${sorted.length} 筆）`;
          moreBtn.onclick = () => {
            const grid = res.querySelector('.sub-card-grid');
            const next = sorted.slice(cardShown, cardShown + CARD_BATCH);
            next.forEach((r, i) => { grid.insertAdjacentHTML('beforeend', mkCard(r, cardShown + i)); });
            cardShown += next.length;
            fitCardHeights(grid);
            if (cardShown >= sorted.length) moreBtn.remove();
            else moreBtn.textContent = `顯示更多（已顯示 ${cardShown} ／ 共 ${sorted.length} 筆）`;
          };
      ...
      const buildCell = r => {
        if (!r.build) return '';
        const b = r.build;
        const tip = `船體：${b.hull[2]}\n舾裝：${b.rigging[2]}\n船首：${b.forecastle[2]}\n船尾：${b.aftcastle[2]}\n承載力：${b.cost}/${b.cap ?? '—'}`;
        const code = p => AIR_RANK_CODE[p[1]] || p[1];
        return `<td data-col="build" class="sub-build-cell"><span data-tip="${tip}">${code(b.hull)}／${code(b.rigging)}／${code(b.forecastle)}／${code(b.aftcastle)}</span></td>`;
      };
      const BATCH = 50; let shown = 0;
      const mkRow = r => {
        const rowStats = r.build ? { sur: r.build.st.sur, ret: r.build.st.ret, fav: r.build.st.fav } : curStats;
        const lootHtml = buildLootCell(r.secIds, rowStats); const thrHtml = buildThresholdStrip(r.secIds, rowStats);
      ...
      let html = legendHtml + `<div class="sub-scroll-hint">← 左右滑動查看全部欄位 →</div><table class="sub-table"><thead><tr>` +
        ...
        thCell('tank', '燃料需求') + (showItemEff ? '' : thCell('count', '目的地數')) + thCell('sec', '目的地') + thCell('loot', '獲得物品') + (hasBuild ? thCell('build', '建議配裝') : '') + `</tr></thead><tbody>`;
    ...
    const lootTier = owned ? (root.querySelector('#air-owned-tier')?.value || 'none') : 'none';
    const lootSortSel = root.querySelector('#air-loot-sort');
    const lootSortMode = (mustLootIds.length && lootSortSel) ? lootSortSel.value : 'exp';
    const runOnMainThread = () => {
      setTimeout(() => {
        if (owned) {
          if (lootSortMode !== 'exp') handleOwnedResult(airFindRoutesForOwnedPartsByLoot(airRank, ownedParts, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootTier, lootMustMeet));
          else handleOwnedResult(airFindRoutesForOwnedParts(airRank, ownedParts, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootTier, lootMustMeet));
        } else {
          if (lootSortMode !== 'exp') renderRoutes(airFindRoutesByLoot(airRank, st, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootMustMeet));
          else renderRoutes(airFindRoutes(airRank, st, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootMustMeet));
        }
      }, 50);
    };

    /* 擁有部件模式現在用「列一次候選路線＋事後查表」，計算量已經跟單一配裝
     * 搜尋差不多，架構比照 sub-panel.js，不再需要 Worker Pool。 */
    ...
    const onMsg = (ev) => {
      ...
      if (ev.data.ok) {
        if (owned) handleOwnedResult({ routes: ev.data.routes, noParts: ev.data.noParts, capped: ev.data.capped, totalCount: ev.data.totalCount, searchedCount: ev.data.searchedCount });
        else renderRoutes(ev.data.routes);
      } else { console.warn('[XIV] worker 計算發生錯誤，改回主執行緒計算：', ev.data.error); runOnMainThread(); }
    };
    worker.addEventListener('message', onMsg);
    worker.postMessage(owned
      ? { reqId, mode: lootSortMode !== 'exp' ? 'ownedByLoot' : 'owned', rank: airRank, ownedParts, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootTier, lootMustMeet }
      : { reqId, mode: lootSortMode !== 'exp' ? 'selectByLoot' : 'select', rank: airRank, stats: st, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootMustMeet });
  };
  ...
  const saveFilterState = () => {
    try {
      localStorage.setItem(FILTER_KEY, JSON.stringify({
        ...
        lootMustMeet: root.querySelector('#air-loot-meet')?.checked || false,
        letterOnly,
        time: {
  ...
  root.querySelector('#air-clear-btn')?.addEventListener('click', () => {
    ...
    const meetChk2 = root.querySelector('#air-loot-meet'); if (meetChk2) meetChk2.checked = false;
  ...
  /* ── 表格式／卡片式 檢視模式切換，架構比照 sub-panel.js ── */
  const applyViewModeSeg = () => {
    root.querySelectorAll('#air-viewmode-seg .sub-seg-btn').forEach(b => { b.classList.toggle('active', b.dataset.mode === viewMode); });
  };
  applyViewModeSeg();
  root.querySelectorAll('#air-viewmode-seg .sub-seg-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      if (viewMode === btn.dataset.mode) return;
      viewMode = btn.dataset.mode;
      try { localStorage.setItem(VIEW_MODE_KEY, viewMode); } catch (e) {}
      applyViewModeSeg();
      if (renderRoutes && currentRoutes.length) renderRoutes(currentRoutes);
    });
  });
  ...
  /* 配裝反查的勾選格只在 buildReversePanel() 時畫過一次，架構比照 sub-panel.js：
     切到「配裝反查」分頁時只同步 checkbox 勾選狀態，不重畫整塊面板。 */
