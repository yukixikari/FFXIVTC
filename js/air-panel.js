...
<div class="sub-filter-col"><label class="sub-lbl">必須獲得物品</label><div class="sub-multisel-wrap" id="air-loot-wrap"></div><label class="sub-chk-lbl sub-loot-strict-lbl"><input type="checkbox" id="air-loot-strict"> 每個目的地都至少有其中一項物品</label><label class="sub-chk-lbl sub-loot-meet-lbl" title="只計入探索性能已經達到取得門檻的物品；還沒達標、理論上有機率但目前配裝拿不到的不算數"><input type="checkbox" id="air-loot-meet"> 忽略未達標的結果</label></div>
  ...
  const doSearch = () => {
    ...
    const renderRoutes = (routes) => {
      ...
      let html = legendHtml + `<div class="sub-scroll-hint">← 左右滑動查看全部欄位 →</div><table class="sub-table"><thead><tr>` +
        ...
        thCell('dist', '距離消耗') + thCell('tank', '燃料需求') + thCell('count', '目的地數') + thCell('sec', '目的地') + thCell('loot', '獲得物品') + (hasBuild ? thCell('build', '建議配裝') : '') + `</tr></thead><tbody>`;
    ...
    const runOnMainThread = () => {
      setTimeout(() => {
        if (owned) handleOwnedResult(airFindRoutesForOwnedParts(airRank, ownedParts, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootTier, lootMustMeet));
    ...
    const onMsg = (ev) => {
      ...
      if (ev.data.ok) {
        if (owned) handleOwnedResult({ routes: ev.data.routes, noParts: ev.data.noParts });
    ...
    worker.postMessage(owned
      ? { reqId, mode: 'owned', rank: airRank, ownedParts, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootTier, lootMustMeet }
      : { reqId, mode: 'select', rank: airRank, stats: st, mustInc, mustExcl, mustLootIds, desiredMins, useTime, lootStrict, lootMustMeet });
  };
