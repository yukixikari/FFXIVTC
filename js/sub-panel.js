...
<div class="sub-top-row"><div class="sub-field-grp"><label class="sub-lbl" for="sub-rank-inp">等級</label><input type="number" id="sub-rank-inp" class="sub-inp" value="1" min="1" max="145"><span class="sub-exp-next" id="sub-exp-next"></span><span class="sub-exp-badge" id="sub-exp-badge" data-tip-wide="1" data-tip="" title="查看全部等級經驗需求">i</span></div><button id="sub-search-btn" class="sub-search-btn" style="margin-left:auto">搜　尋</button><button id="sub-clear-btn" class="sub-clear-btn">清除篩選</button><button id="sub-parts-mode-toggle" class="sub-view-btn" type="button">切換「擁有部件」選項</button><button id="sub-view-toggle" class="sub-view-btn" type="button">切換為精簡模式</button><button id="sub-map-btn" class="sub-view-btn" type="button">探索地圖</button></div>
...
<div class="sub-filter-col"><label class="sub-lbl">必須獲得物品</label><div class="sub-multisel-wrap" id="sub-loot-wrap"></div><label class="sub-chk-lbl sub-loot-strict-lbl"><input type="checkbox" id="sub-loot-strict"> 每個目的地都至少有其中一項物品</label><label class="sub-chk-lbl sub-loot-meet-lbl" title="只計入探索性能已經達到取得門檻的物品；還沒達標、理論上有機率但目前配裝拿不到的不算數"><input type="checkbox" id="sub-loot-meet"> 忽略未達標的結果</label></div>
...
<div id="sub-reverse-panel" style="display:none"></div>
    ...
    const doSearch=()=>{
      ...
      /* 擁有部件模式現在改用「列一次候選路線＋事後查表」（見 sub-calc.js 的
       * subFindRoutesForOwnedParts 說明），計算量已經跟單一配裝搜尋差不多，
       * 不再需要 Worker Pool 平行、裁切組合數這些應急措施，跟下面「單一配裝」
       * 走同一條單一 Worker 路徑即可。 */
      ...
      worker.postMessage(owned
        ?{reqId,mode:'owned',rank:subRank,ownedParts,mustInc,mustExcl,mustIncMaps,mustExclMaps,mustLootIds,desiredMins,useTime,letterOnly,lootStrict,lootTier,lootMustMeet}
