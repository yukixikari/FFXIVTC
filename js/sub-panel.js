...
<div id="sub-reverse-panel" style="display:none"></div>
    ...
    const doSearch=()=>{
      ...
      /* 擁有部件模式：候選路線的「列舉」跟配裝無關、只跟地圖有關，可以照地圖
       * 切開分給多個 Worker 平行列舉；列完在主執行緒合併，再丟給一個 Worker
       * 做「依擁有部件查表」。地圖數只有 1 張時（或平行池不可用時）沒有東西
       * 好切，直接走單一 Worker 全流程即可。 */
      if(owned){
      ...
      worker.addEventListener('message',onMsg);
      worker.postMessage({reqId,mode:'select',rank:subRank,stats:st,mustInc,mustExcl,mustIncMaps,mustExclMaps,mustLootIds,desiredMins,useTime,letterOnly,lootStrict,lootMustMeet});
    };
