...
self.onmessage = function (e) {
  ...
  try {
    if (d.mode === 'owned') {
      const result = airFindRoutesForOwnedParts(
        d.rank, d.ownedParts, d.mustInc, d.mustExcl,
        d.mustLootIds, d.desiredMins, d.useTime, d.lootStrict, d.lootTier, d.lootMustMeet
      );
      ...
      const result = airFindRoutesForOwnedPartsByLoot(
        d.rank, d.ownedParts, d.mustInc, d.mustExcl,
        d.mustLootIds, d.desiredMins, d.useTime, d.lootStrict, d.lootTier, d.lootMustMeet
      );
      self.postMessage({ reqId: reqId, ok: true, routes: result.routes, noParts: result.noParts, capped: result.capped, totalCount: result.totalCount, searchedCount: result.searchedCount });
    } else if (d.mode === 'selectByLoot') {
      const routes = airFindRoutesByLoot(
        d.rank, d.stats, d.mustInc, d.mustExcl,
        d.mustLootIds, d.desiredMins, d.useTime, d.lootStrict, d.lootMustMeet
      );
